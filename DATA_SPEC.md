...
**현재 상태**: `data/auctions.json` 에 실제 낙찰 물건 **4,510건**이 들어가 있습니다.
...
※ `kind` 는 `usage_taxonomy.json` 의 잎 이름과 **정확히 같은 문자열**이어야 합니다 (선택 화면이 계열 → 종 → 용도로 필터링).
현재 40종이 1:1로 맞물려 있으며, 새 용도가 생기면 `usage_taxonomy.json` 에도 추가해야 그 용도를 고를 수 있습니다.
